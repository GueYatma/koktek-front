...
- `reviewed` : validé éditorialement
- `published` : publié
...
4. Relire dans Directus et passer `manual_review_status` à `reviewed` quand le contenu est validé
5. Corriger titre, chapo, intertitres, méta SEO si besoin
6. Publier ensuite dans `blog_posts`

---

## Configuration Directus live

La configuration Directus de production a été alignée avec ce workflow le 11 mars 2026.

### `blog_posts`

- icône : `newspaper`
- preview URL : `https://koktek.com/blog/{{slug}}`
- `cover_image` : accepte une URL externe absolue ou un asset local Directus
- `status`, `manual_review_status`, `pillar`, `article_type`, `search_intent`, `seasonality` : interfaces `select-dropdown`
- `reading_time` : lecture seule

### `seo_topics`

- icône : `edit_note`
- `status`, `pillar`, `article_type`, `search_intent`, `seasonality` : interfaces `select-dropdown`
- `generated_post_id`, `last_generated_at`, `created_at`, `updated_at` : lecture seule
- `manual_product_ids` et `product_filters` : interfaces JSON (`input-code`)

Cette configuration vise à rendre le flux éditorial exploitable sans repasser par SQL pour les opérations courantes.
